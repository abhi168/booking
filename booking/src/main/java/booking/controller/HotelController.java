...
public class HotelController {
	...
	@RequestMapping(value = { "/search/", "/search" }, method = RequestMethod.GET) // "/search",
	// public String searchHotel(Model model, @RequestParam("searchString")
	// String searchString,
	// @RequestParam("rating") int rating, @RequestParam("ac") boolean ac) {
	public String searchHotel(Model model, @RequestParam Map<String, String> allRequestParams) {
		String searchString = allRequestParams.get("searchString");
		String rating = allRequestParams.get("rating");
		String ac = allRequestParams.get("ac");
		...
		while (ithotels.hasNext()) {
			...
			if (h.getName().toLowerCase().contains(searchString.toLowerCase())
					&& h.getRating() > Integer.parseInt(rating) && h.isAc() == Boolean.parseBoolean(ac))
				if (location.contains("All"))
					hotelsList.add(h);
				else if (h.getAddress().toLowerCase().contains(location.toLowerCase())) {
					hotelsList.add(h);
				}
		}
		/*
		 ...
		 * 
		 * @Override public int compare(Hotel o1, Hotel o2) { return
		 * Double.compare(o1.getRating(), o2.getRating()); } });
		 */

		model.addAttribute("hotels", hotelsList);
		model.addAttribute("str", allRequestParams);
		return "hotels/index";
	}
	...
	public String hotelMap(@PathVariable("id") long id, Model model, @ModelAttribute Booking booking) {
		...
		model.addAttribute("occupancy",
				getOccupancy(hotels.findOne(id), booking.getBegin_date(), booking.getEnd_date()));
		return "hotels/map";
	}
